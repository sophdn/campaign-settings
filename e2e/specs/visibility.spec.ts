...
test('an ungranted player sees neither the restricted entity in the list nor the wiki', async ({
  ...
  // not in the wiki index either (loaded once the public npc shows). The index
  // moved off the world root to /wiki when the dashboard took the root.
...
/**
 * The graph is a view of the wiki page, not its own route — toggle into it. The
 * wiki moved off the world root to `/wiki` when the dashboard took the root.
 */
