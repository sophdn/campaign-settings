 ...
 */
import { CONTENT_REPOS, ENTITY_REPOS } from '../packages/server/src/data/content-repos'
...
import { createTouch } from '../packages/server/src/data/touches'
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // The dashboard fixture: a session that touched the seeded NPC, and a
    // character player1 plays. Both are public, so every member sees them —
    // what the dashboard spec is about is ROLE, not visibility.
    const sessionRepo = ENTITY_REPOS.session
    if (!sessionRepo) throw new Error('session repo is not registered')
    const session = await sessionRepo.create(ownerCtx, {
      name: SEED_SESSION.name,
      played_at: SEED_SESSION.playedAt,
      captured_text: 'They took the harbour job.',
      visibility: 'public',
    })
    await createTouch(ownerCtx, {
      session_id: session.id,
      entity_id: seedNpc.id,
      touch_type: 'met',
    })
    // UNLINKED on purpose. pc-player-link.spec.ts releases every seat in this
    // world after each of its tests, because the world holds exactly two
    // players and a leftover link makes the NEXT test fail for a reason that
    // has nothing to do with what it checks. A seeded link would be exactly
    // that leftover. The dashboard spec links this character itself and
    // releases it again, which keeps both specs honest about what they set up.
    const pcRepo = CONTENT_REPOS.pc
    if (!pcRepo) throw new Error('pc content repo is not registered')
    await pcRepo.create(ownerCtx, { name: SEED_PC.name, visibility: 'public' })
