 ...
 * downloadable file. Assembled from `listWorlds` + the notes route, both of
 * which already return exactly the caller's own rows — so this cannot read
 * anything the account could not already read.
 *
 * Character sheets used to be part of this. Since migration 0018 a character is
 * the DM's PC page pointing at the player, not the player's row, so deleting an
 * account clears the pointer and leaves the world's write-up standing.
 *
