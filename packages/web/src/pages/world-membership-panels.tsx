...
/**
 * Build the leaver's own data as a downloadable file, from the route that
 * already returns exactly their own rows.
 *
 * Notes are all that is left to take. Character sheets used to ride along here;
 * since migration 0018 a character is the DM's own PC page that merely points
 * at the player, so it is not the leaver's to export and leaving only clears
 * the pointer.
 *
 * Deliberately NOT a new server endpoint. `listNotes` is already scoped to the
 * caller by `authz/player-data.ts`, so assembling the file here cannot widen
 * what anyone can read — whereas a new "export" route is precisely where a
 * departing player's download quietly becomes the world's.
 */
async function buildMyDataFile(
  api: ApiClient,
  worldId: string,
): Promise<{ url: string; filename: string }> {
  const notes = await api.listNotes(worldId)
  const blob = new Blob([JSON.stringify({ worldId, notes }, null, 2)], {
