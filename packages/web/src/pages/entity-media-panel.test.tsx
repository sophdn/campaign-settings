...
describe('EntityMediaPanel — which image leads the page', () => {
  it('marks the main image for EVERY viewer, not only the owner', async () => {
    const api = makeApi({
      listEntityMedia: vi.fn(() =>
        Promise.resolve([attachment({ id: 'm1', is_primary: true }), attachment({ id: 'm2' })]),
      ),
    })
    mount(api, false)
    // A player looking at a gallery of several should be able to tell which one
    // is the portrait they saw at the top of the page.
    expect(await screen.findByText('Main image')).toBeTruthy()
    expect(screen.getAllByText('Main image')).toHaveLength(1)
  })

  it('promotes an already-attached image, which the avatar’s plus cannot do', async () => {
    const setPrimaryMedia = vi.fn(() => Promise.resolve(attachment({ is_primary: true })))
    const onPrimaryChanged = vi.fn()
    render(
      <ApiProvider
        value={makeApi({
          listEntityMedia: vi.fn(() => Promise.resolve([attachment({ id: 'm2' })])),
          setPrimaryMedia,
        })}
      >
        <ModalProvider>
    ...
    fireEvent.click(await screen.findByRole('button', { name: 'Use mira.png as the main image' }))
    await waitFor(() => expect(setPrimaryMedia).toHaveBeenCalledWith('w1', 'npc', 'e1', 'm2'))
    // The avatar above has its own read, so it is told rather than left stale.
    await waitFor(() => expect(onPrimaryChanged).toHaveBeenCalled())
  })

  it('stands the main image down by clearing it, not by deleting the file', async () => {
    const setPrimaryMedia = vi.fn(() => Promise.resolve(null))
    const deleteMedia = vi.fn(() => Promise.resolve())
    mount(
      makeApi({
        listEntityMedia: vi.fn(() => Promise.resolve([attachment({ id: 'm1', is_primary: true })])),
        setPrimaryMedia,
        deleteMedia,
      }),
    )
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: 'Stop using mira.png as the main image' }),
    )
    await waitFor(() => expect(setPrimaryMedia).toHaveBeenCalledWith('w1', 'npc', 'e1', null))
    expect(deleteMedia).not.toHaveBeenCalled()
  })

  it('offers a player no way to change it', async () => {
    mount(
      makeApi({
        listEntityMedia: vi.fn(() => Promise.resolve([attachment({ is_primary: true })])),
      }),
      false,
    )
    await screen.findByText('Main image')
    expect(screen.queryByRole('button', { name: /main image$/ })).toBeNull()
  })
