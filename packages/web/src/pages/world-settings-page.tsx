...
export function WorldSettingsPage(): React.JSX.Element {
  ...
  const { worldId, worldName, refreshWorld } = useWorld()
  ...
  if (!isOwner) {
    return (
      <div>
        ...
        {/* Calendars are world config, readable by every member — so a player
            sees the world's date scheme here, without any control to change it. */}
        <CalendarsPanel worldId={worldId} canEdit={false} />
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <CalendarsPanel worldId={worldId} canEdit />
