import type { CalendarConfig, CalendarKind, Visibility } from '@campaign-settings/shared'
...
export interface ApiClient {
  ...
  getDashboard(worldId: string): Promise<WorldDashboard>
  ...
  /** THE image for an entity, or null. One row rather than the whole gallery. */
  getPrimaryMedia(worldId: string, kind: string, id: string): Promise<MediaAttachment | null>
  /** Make one attachment THE image, or pass null to clear it without deleting. */
  setPrimaryMedia(
    worldId: string,
    kind: string,
    id: string,
  ...
  createRelationship(
    ...
    kind: string,
    id: string,
    input: { toId: string; type: string; note?: string; qualifier?: string },
  ): Promise<EntityRelationship>
  /**
   * Specify an existing relationship further: change its type, add a qualifier
   * or a note. The reason this exists is the bracket-derived row — a `[[link]]`
   * creates one at "Related to", and saying what the link IS must not mean
   * deleting it, which would lose the reveal that governs who may see it.
   */
  updateRelationship(
    worldId: string,
    id: string,
    patch: { type?: string; note?: string; qualifier?: string | null },
  ): Promise<EntityRelationship>
   ...
   *
   * Filtered by BOTH visibility rules server-side — the row's own, and the
   * currency's — so nothing here needs to filter and a row naming a hidden
   * currency simply is not in the list.
   */
  listCurrencyAttachments(
    worldId: string,
    ownerKind: CurrencyOwnerKind,
    ownerId: string,
  ): Promise<CurrencyAttachment[]>
  /** Attach a currency to an owner. Owner-only server-side; a repeat is a 409. */
  attachCurrency(
    worldId: string,
    ownerKind: CurrencyOwnerKind,
    ownerId: string,
  ...
  /**
   * Edit one. Setting `isPrimary` demotes the owner's incumbent primary in the
   * same transaction, so the caller never has to demote first.
   *
   * `ownerKind` names which of the two TABLES the id lives in, not a redundant
   * copy of the owner: both mint ids from the same generator.
   */
  updateCurrencyAttachment(
    worldId: string,
    ownerKind: CurrencyOwnerKind,
    id: string,
    patch: { isPrimary?: boolean; notes?: string; visibility?: 'public' | 'dm_only' },
  ): Promise<CurrencyAttachment>
  /** Detach. Hard-deletes: an attachment is a link, and there is no trash for one. */
  detachCurrency(worldId: string, ownerKind: CurrencyOwnerKind, id: string): Promise<void>
  /** The settlements and organizations that use a currency — the inverse read. */
  listCurrencyUsers(worldId: string, currencyId: string): Promise<CurrencyUser[]>

  /**
   * The world's calendars. Readable by every member — a calendar is world config,
   * like the world's name, so this is not filtered per-actor.
   */
  listCalendars(worldId: string): Promise<WorldCalendar[]>
  /** The one calendar dates render through, or null when the world has none. */
  activeCalendar(worldId: string): Promise<WorldCalendar | null>
  /** Owner-only server-side. Created inactive: activating is its own act. */
  createCalendar(
    worldId: string,
    input: { name: string; kind: CalendarKind; config?: CalendarConfig },
  ): Promise<WorldCalendar>
  updateCalendar(
    worldId: string,
    id: string,
    patch: { name?: string; kind?: CalendarKind; config?: CalendarConfig },
  ): Promise<WorldCalendar>
  /** Make this the world's active calendar; the previous one is cleared. */
  activateCalendar(worldId: string, id: string): Promise<WorldCalendar[]>
  deleteCalendar(worldId: string, id: string): Promise<void>
...
export function createApiClient(baseUrl = ''): ApiClient {
  ...
  return {
    ...
    async getDashboard(worldId) {
      return (await request<{ dashboard: WorldDashboard }>('GET', `${world(worldId)}/dashboard`))
        .dashboard
    },
    ...
    async getPrimaryMedia(worldId, kind, id) {
      return (
        await request<{ media: MediaAttachment | null }>(
          'GET',
          `${world(worldId)}/entities/${seg(kind)}/${seg(id)}/media/primary`,
        )
      ).media
    },
    async setPrimaryMedia(worldId, kind, id, mediaId) {
      return (
        await request<{ media: MediaAttachment | null }>(
          'PUT',
          `${world(worldId)}/entities/${seg(kind)}/${seg(id)}/media/primary`,
    ...
    async updateRelationship(worldId, id, patch) {
      return (
        await request<{ relationship: EntityRelationship }>(
          'PATCH',
          `${world(worldId)}/relationships/${seg(id)}`,
          patch,
        )
      ).relationship
    },
    ...
    async listCurrencyAttachments(worldId, ownerKind, ownerId) {
      return (
        await request<{ attachments: CurrencyAttachment[] }>(
          'GET',
          `${world(worldId)}/entities/${seg(ownerKind)}/${seg(ownerId)}/currencies`,
        )
      ).attachments
    },
    async attachCurrency(worldId, ownerKind, ownerId, input) {
      return (
        await request<{ attachment: CurrencyAttachment }>(
          'POST',
          `${world(worldId)}/entities/${seg(ownerKind)}/${seg(ownerId)}/currencies`,
          input,
        )
    ...
    async updateCurrencyAttachment(worldId, ownerKind, id, patch) {
      return (
        await request<{ attachment: CurrencyAttachment }>(
          'PATCH',
          `${world(worldId)}/currency-attachments/${seg(ownerKind)}/${seg(id)}`,
          patch,
        )
      ).attachment
    },
    async detachCurrency(worldId, ownerKind, id) {
      await request('DELETE', `${world(worldId)}/currency-attachments/${seg(ownerKind)}/${seg(id)}`)
    },
    async listCurrencyUsers(worldId, currencyId) {
      return (
        await request<{ users: CurrencyUser[] }>(
          'GET',
          `${world(worldId)}/currencies/${seg(currencyId)}/users`,
        )
      ).users
    },
    ...
    async listCalendars(worldId) {
      return (await request<{ calendars: WorldCalendar[] }>('GET', `${world(worldId)}/calendars`))
        .calendars
    },
    async activeCalendar(worldId) {
      return (
        await request<{ calendar: WorldCalendar | null }>(
          'GET',
          `${world(worldId)}/calendars/active`,
        )
      ).calendar
    },
    async createCalendar(worldId, input) {
      return (
        await request<{ calendar: WorldCalendar }>('POST', `${world(worldId)}/calendars`, input)
      ).calendar
    },
    async updateCalendar(worldId, id, patch) {
      return (
        await request<{ calendar: WorldCalendar }>(
          'PATCH',
          `${world(worldId)}/calendars/${seg(id)}`,
          patch,
        )
      ).calendar
    },
    async activateCalendar(worldId, id) {
      return (
        await request<{ calendars: WorldCalendar[] }>(
          'POST',
          `${world(worldId)}/calendars/${seg(id)}/activate`,
        )
      ).calendars
    },
    async deleteCalendar(worldId, id) {
      await request('DELETE', `${world(worldId)}/calendars/${seg(id)}`)
    },
