...
describe('maps, pins and relationships', () => {
  ...
  it('routes the currency-attachment calls, with the by-id writes off the owner path', async () => {
    // The forward reads hang off the OWNER; editing and detaching key off the
    // attachment's own id and so sit on a flat path, carrying the owner KIND
    // because it names which of the two tables the id lives in. Nesting them
    // would put a static `currencies` segment beside `:id` at the same level,
    // where an entity whose id is literally "currencies" routes to the wrong
    // handler — which is why the URLs below are the assertion.
    const calls = stubFetch(
      { body: { attachments: [] } },
      { body: { attachment: { id: 'ca1' } } },
      { body: { attachment: { id: 'ca1' } } },
      { body: { ok: true } },
      { body: { users: [] } },
    )
    ...
    expect(await api.listCurrencyAttachments('w1', 'settlement', 'st1')).toEqual([])
    expect((await api.attachCurrency('w1', 'settlement', 'st1', { currencyId: 'cu1' })).id).toBe(
      'ca1',
    )
    expect(
      (await api.updateCurrencyAttachment('w1', 'settlement', 'ca1', { isPrimary: true })).id,
    ).toBe('ca1')
    await api.detachCurrency('w1', 'organization', 'ca2')
    expect(await api.listCurrencyUsers('w1', 'cu1')).toEqual([])

    expect(calls.map((c) => c.url)).toEqual([
      '/api/worlds/w1/entities/settlement/st1/currencies',
      '/api/worlds/w1/entities/settlement/st1/currencies',
      '/api/worlds/w1/currency-attachments/settlement/ca1',
      '/api/worlds/w1/currency-attachments/organization/ca2',
      '/api/worlds/w1/currencies/cu1/users',
    ])
    expect(calls.map((c) => c.init.method)).toEqual(['GET', 'POST', 'PATCH', 'DELETE', 'GET'])
  })
})
