...
describe('player-data authorization', () => {
  ...
  it('applied to player_characters: a player reads only their own; the DM reads all', async () => {
    await withTestDatabase(async (pool) => {
      ...
      await db
        ...
        .values({ id: newId(), world_id: world.id, owner_id: p1, name: 'C1' })
        .execute()
      await db
        .insertInto('player_characters')
        .values({ id: newId(), world_id: world.id, owner_id: p2, name: 'C2' })
        .execute()

      // how a player-data repo (task 13) will apply the scope
      async function read(ctx: WorldContext) {
        let q = db.selectFrom('player_characters').selectAll().where('world_id', '=', world.id)
      ...
      expect(ownRows[0]?.owner_id).toBe(p1)
