...
export function createContentRepository<TB extends ContentTableName>(
  ...
  /**
   * Constrain a WRITE to the kind this repo serves.
   *
   * `visible()` carries the kind filter for every READ, but a write states its
   * own `where` clauses and so does not inherit it. Left unfixed that means
   * reads and writes disagree about what the `:kind` URL segment means:
   * `DELETE /trash/species/<an npc id>` destroys the NPC, and
   * `PATCH /entities/species/<an npc id>` sets its `name`/`visibility` — a
   * mutation of a row the owner is not looking at, which no confirm dialog can
   * catch (bug 1219).
   *
   * So all four writes route their `where` through here. It is a clause rather
   * than a preflight `select` because the database can answer "and is it this
   * kind" inside the statement that already runs; a separate lookup would add a
   * round trip to every edit and open a window between the check and the write.
   * A miss therefore looks exactly like a miss on `world_id` or `deleted_at` —
   * no row updated — and every caller already turns that into a 404.
   *
   * A repo over a bespoke table (`sessions`, `maps`, `entity_passages`) serves
   * exactly one kind by construction, so there is nothing to constrain and this
   * returns the query untouched.
   */
  function ofThisKind<Q extends { where(lhs: 'kind', op: '=', rhs: string): Q }>(q: Q): Q {
    if (!isEntities || kind === undefined) return q
  ...
  return {
    ...
    /**
     * Soft-delete; returns whether a live row of THIS kind was actually deleted.
     * False for an id belonging to another kind — see `ofThisKind`.
     */
    async softDelete(ctx: WorldContext, id: string): Promise<boolean> {
      assertContentWrite(ctx)
      const view = ctx.db as unknown as Kysely<ContentView>
      const res = await ofThisKind(
        view
          .updateTable(name)
