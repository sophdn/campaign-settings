...
describe('content seam — writes agree with reads about the kind (bug 1219)', () => {
  it('an id of another kind is a miss for update and softDelete, and the row is untouched', async () => {
    await withTestDatabase(async (pool) => {
      const db = createDb(pool)
      await migrateToLatest(db)
      const { ownerCtx } = await setupWorld(db, 'k')
      const species = CONTENT_REPOS.species!

      const npc = await createNpc(ownerCtx, { name: 'Aelin', visibility: 'dm_only' })

      // The read door already refuses this id under the wrong kind...
      expect(await species.get(ownerCtx, npc.id)).toBeUndefined()

      // ...and now so do both write doors, in the shape their routes 404 on.
      expect(await species.update(ownerCtx, npc.id, { name: 'x', visibility: 'public' })).toBe(
        undefined,
      )
      expect(await species.softDelete(ownerCtx, npc.id)).toBe(false)

      // Nothing landed — `visibility` in particular, which the miss could
      // otherwise have flipped on a row the caller was not looking at.
      expect(await getNpc(ownerCtx, npc.id)).toMatchObject({
        name: 'Aelin',
        visibility: 'dm_only',
      })
    })
  ...
  it('the right kind still updates and deletes normally', async () => {
    await withTestDatabase(async (pool) => {
      const db = createDb(pool)
      await migrateToLatest(db)
      const { ownerCtx } = await setupWorld(db, 'k2')
      const species = CONTENT_REPOS.species!

      const elf = await species.create(ownerCtx, { name: 'Elf' })
      expect(await species.update(ownerCtx, elf.id, { name: 'High Elf' })).toMatchObject({
        name: 'High Elf',
      })
      expect(await species.softDelete(ownerCtx, elf.id)).toBe(true)
    })
  })
})
