...
/**
 * Default ceiling on account lookups. An exact-match lookup is still an
 * enumeration oracle if it can be called without limit, so it is capped even
 * though it is already owner-gated. Sized for a human adding players (a
 * handful per sitting), not for a script walking a name list.
 */
export const DEFAULT_LOOKUP_RATE_LIMIT = { max: 20, timeWindow: 60_000 }
...
export interface AppDeps {
  ...
  now?: () => Date
  /**
   * Ceiling on account-lookup calls per caller. Optional; defaults to
   * {@link DEFAULT_LOOKUP_RATE_LIMIT}. Tests lower it so the limit is exercised
   * without firing the production quota's worth of requests.
   */
  lookupRateLimit?: { max: number; timeWindow: number }
  /**
...
const CharCreateBody = z.object({
  name: z.string().min(1),
  data: z.record(z.string(), z.unknown()).optional(),
})
const CharPatchBody = z.object({
...
export function buildApp(deps: AppDeps): FastifyInstance {
  ...
  const accountManaged = {
    preHandler: [
      requireAccount,
      // Explicitly async: fastify decides callback-vs-promise style from the
      // hook's shape, and a plain sync arrow here silently never completes.
      async (): Promise<void> => {
   ...
   *
   * Rate limiting and the wider abuse guards are deliberately NOT here: task
   * 3551 owns them so the whole public surface is hardened as one piece. Until
   * then the flag being off is what keeps this endpoint unreachable.
   */
  ...
  app.post('/api/demo-login', async (req, reply) => {
    ...
    setSessionCookie(reply, sessionId, cookieOpts)
    return { account: { id: accountId, username: demoUsername } }
  })
  app.get('/api/me', authed, async (req) => ({ account: accountOf(req) }))

  // ── password reset (unauthenticated) ──
  app.post('/api/password-reset/request', async (req) => {
  ...
  app.post('/api/password-reset/confirm', async (req) => {
    requireSurface(flags.passwordResetEnabled, 'password reset')
  ...
  app.post('/api/verify-email', async (req) => {
    ...
    if (!(await consumeVerificationToken(deps.db, token, clock()))) throw invalidVerificationToken()
    return { ok: true }
  })
  ...
  app.after(() => {
    app.get(
      ...
      { ...inWorld, config: { rateLimit: deps.lookupRateLimit ?? DEFAULT_LOOKUP_RATE_LIMIT } },
      async (req) => {
  ...
  app.post('/api/worlds/:worldId/entities/:kind', inWorld, async (req, reply) => {
    ...
    const entity = await repoOf(param(req, 'kind')).create(
      ctxOf(req),
      EntityCreateBody.parse(req.body),
    )
  ...
  app.patch('/api/worlds/:worldId/entities/:kind/:id', inWorld, async (req) => {
    const entity = await repoOf(param(req, 'kind')).update(
      ctxOf(req),
      param(req, 'id'),
      PatchBody.parse(req.body),
    )
    if (!entity) throw notFound('entity')
    return { entity }
  })
  ...
  app.get('/api/worlds/:worldId/characters', inWorld, async (req) => ({
    characters: await listCharacters(ctxOf(req)),
  }))
  app.post('/api/worlds/:worldId/characters', inWorld, async (req, reply) => {
    const b = CharCreateBody.parse(req.body)
    const input = b.data !== undefined ? { name: b.name, data: b.data } : { name: b.name }
    const character = await createCharacter(ctxOf(req), input)
    return reply.code(201).send({ character })
  })
  app.patch('/api/worlds/:worldId/characters/:id', inWorld, async (req) => {
    const b = CharPatchBody.parse(req.body)
    const patch: { name?: string; data?: Record<string, unknown> } = {}
    if (b.name !== undefined) patch.name = b.name
    if (b.data !== undefined) patch.data = b.data
    const character = await updateCharacter(ctxOf(req), param(req, 'id'), patch)
    if (!character) throw notFound('character')
    return { character }
  })
  app.delete('/api/worlds/:worldId/characters/:id', inWorld, async (req) => {
    if (!(await deleteCharacter(ctxOf(req), param(req, 'id')))) throw notFound('character')
    return { ok: true }
  })
