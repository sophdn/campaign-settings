...
describe('demo read-only enforcement', () => {
  ...
  /**
   * The account family is refused for the demo principal even with
   * `accountManagementEnabled` ON, which is the configuration this closes: the
   * flag is a deployment choice an operator can get wrong, and the e2e harness
   * genuinely wants it on. `GET /api/account/sessions` would otherwise hand one
   * visitor the device label of the browser that opened the shared session.
   */
  it('refuses the demo principal the whole account family, reads included', async () => {
    await withTestDatabase(async (pool) => {
      const { app } = await setup(pool, openFlags())
      const demo = cookieOf(await demoLogin(app))

      for (const url of ['/api/account/sessions', '/api/account/status']) {
        const res = await app.inject({ method: 'GET', url, headers: { cookie: demo } })
        expect(res.statusCode, url).toBe(403)
        expect(res.json().error.code, url).toBe('demo_read_only')
      }
    })
  })

  it('lets a NORMAL player read the same account routes — this is the principal, not the flag', async () => {
    await withTestDatabase(async (pool) => {
      const { app, player } = await setup(pool, openFlags())

      for (const url of ['/api/account/sessions', '/api/account/status']) {
        expect(
          (await app.inject({ method: 'GET', url, headers: { cookie: player } })).statusCode,
          url,
        ).toBe(200)
      }
    })
  ...
  it('tells the SPA which principal it is, so it can stop offering that page', async () => {
    await withTestDatabase(async (pool) => {
      const { app, player } = await setup(pool)
      const demo = cookieOf(await demoLogin(app))

      const asDemo = await app.inject({ method: 'GET', url: '/api/me', headers: { cookie: demo } })
      expect(asDemo.json().account.isDemo).toBe(true)
      const asPlayer = await app.inject({
        method: 'GET',
        url: '/api/me',
        headers: { cookie: player },
      })
      expect(asPlayer.json().account.isDemo).toBe(false)
      // and the sign-in response says so too, since the SPA adopts it directly
      expect((await demoLogin(app)).json().account.isDemo).toBe(true)
    })
  })
