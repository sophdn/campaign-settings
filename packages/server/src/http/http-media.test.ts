...
describe('the primary-image routes', () => {
  it('set, read, replace, clear — and a player is refused', async () => {
    await withTestDatabase(async (pool: Pool) => {
      const uploadsDir = mkdtempSync(join(tmpdir(), 'cs-uploads-'))
      const db = createDb(pool)
      await migrateToLatest(db)
      ...
      try {
        ...
        const npcId = npc.json().entity.id as string
        const dmAccount = await db
          .selectFrom('accounts')
          .select('id')
          .where('username', '=', 'dm')
        ...
        const attach = (name: string) =>
          createMediaAttachment(ctx, {
            ...
            owner_id: npcId,
            media_kind: 'image',
            file_path: join('w', name),
            original_filename: name,
            mime_type: 'image/png',
            byte_size: PNG.length,
          })
        const a = await attach('a.png')
        const b = await attach('b.png')

        const primaryUrl = `/api/worlds/${slug}/entities/npc/${npcId}/media/primary`
        const readPrimary = (cookie: string) =>
          app.inject({ method: 'GET', url: primaryUrl, headers: { cookie } })
        const setPrimary = (cookie: string, mediaId: string | null) =>
          app.inject({
            method: 'PUT',
            url: primaryUrl,
            headers: { cookie },
        ...
        // Nothing nominated yet: null, not a 404. An entity with no main image
        // is the ordinary case and the page renders a placeholder for it.
        expect((await readPrimary(dm)).json().media).toBeNull()

        expect((await setPrimary(dm, a.id)).statusCode).toBe(200)
        expect((await readPrimary(dm)).json().media.id).toBe(a.id)
        // A player READS it — the image leads a page they are entitled to see.
        expect((await readPrimary(playerCookie)).json().media.id).toBe(a.id)

        // Replacing moves the seat rather than colliding with the index.
        expect((await setPrimary(dm, b.id)).statusCode).toBe(200)
        expect((await readPrimary(dm)).json().media.id).toBe(b.id)

        // Clearing leaves the attachment in the gallery.
        expect((await setPrimary(dm, null)).statusCode).toBe(200)
        expect((await readPrimary(dm)).json().media).toBeNull()
        const list = await app.inject({
          method: 'GET',
          url: `/api/worlds/${slug}/entities/npc/${npcId}/media`,
          headers: { cookie: dm },
        })
        expect(list.json().media).toHaveLength(2)

        // An unknown id is a 404, not a silent no-op.
        expect((await setPrimary(dm, 'no-such-media')).statusCode).toBe(404)

        // A PLAYER is refused by the server. Hiding the plus is presentation.
        expect((await setPrimary(playerCookie, a.id)).statusCode).toBe(403)
      } finally {
        await app.close()
        rmSync(uploadsDir, { recursive: true, force: true })
      }
