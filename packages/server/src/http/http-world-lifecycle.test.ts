...
async function setup(pool: Pool) {
  ...
  // A PC page the DM authors and links to the player — the post-0018 shape of
  // "this player's character". Created by the DM, because content writes are
  // owner-only and the link is the DM's to set.
  const pc = await app.inject({
    method: 'POST',
    url: `${base}/entities/pc`,
    headers: { cookie: dm },
    payload: { name: 'My PC', account_id: playerAccount.id },
  })
...
describe('renaming a world', () => {
  ...
  it('carries the world’s people and content across with it', async () => {
    await withTestDatabase(async (pool) => {
      ...
      // The NPC and the PC the setup creates — a rename moves the whole wiki,
      // not a sample of it.
      expect(wiki.json().entries).toHaveLength(2)
...
describe('leaving a world', () => {
  ...
  it('deletes the leaver’s notes and grants, and UNLINKS their PC without deleting it', async () => {
    await withTestDatabase(async (pool) => {
      const { app, db, base, dm, player, playerId, entityId, pcId, worldId } = await setup(pool)
      ...
      // The page is the DM's and survives; only the pointer at the departed
      // player is cleared. Read back through the API rather than the table, so
      // this asserts what the DM actually sees.
      const after = await app.inject({
        method: 'GET',
        url: `${base}/entities/pc/${pcId}`,
        headers: { cookie: dm },
      })
      expect(after.statusCode).toBe(200)
      expect(after.json().entity.name).toBe('My PC')
      expect(after.json().entity.account_id).toBeNull()
    })
  })
