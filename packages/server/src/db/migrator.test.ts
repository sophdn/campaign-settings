...
describe('migrations 0001–0005 — full schema port + class-table inheritance', () => {
  it('builds every table with world_id scoping, Postgres-native types, FKs/enums; down() reverses it', async () => {
    await withTestDatabase(async (pool) => {
      ...
      // — world_id is on the base, the detail tables, junctions, meta — not auth roots —
      for (const t of [
        'entities',
        'npc_details',
        'npc_languages',
      ...
      await db
        ...
        .values({ world_id: 'w1', npc_id: 'npc1', language_id: 'lg1', role: 'native' })
        .execute()
      await db
        .insertInto('culture_languages')
        .values({ world_id: 'w1', culture_id: 'cu1', language_id: 'lg1', role: 'liturgical' })
        .execute()
