...
/**
 * Make one attachment THE image for its owner, or clear the owner's primary.
 *
 * Two statements in ONE transaction: clear whatever holds the seat, then set
 * the new one. That order matters — 0020's partial unique index permits exactly
 * one primary per owner, so setting before clearing would collide with the row
 * being replaced. Doing it the other way round inside a transaction means a
 * reader never observes an owner with two primaries, or with none in the middle
 * of a change.
 *
 * The clear-only case (`id` null) is not an edge: an owner who decides no image
 * should lead the page has to be able to say so without deleting the file.
 *
 * Returns false when `id` names no live attachment of this owner, so the route
 * answers 404 rather than reporting success for an id that was never there. A
 * cross-owner id is the same answer as a missing one, deliberately: "make this
 * OTHER entity's photo my portrait" is not a request with a sensible outcome.
 */
export async function setPrimaryMedia(
  ...
  assertContentWrite(ctx)
  if (id !== null) {
    const target = await getMediaById(ctx, id)
    if (!target || target.owner_kind !== ownerKind || target.owner_id !== ownerId) return false
  }
...
/**
 * The one image that leads an owner's page, if it has one.
 *
 * Its own query rather than a filter over {@link listMediaForOwner}: the entity
 * page renders the avatar before anything else and does not otherwise need the
 * gallery, so fetching every attachment to pick one would make a page with
 * twenty photographs pay for nineteen it will not draw.
 */
export function findPrimaryMedia(
  ctx: WorldContext,
  ownerKind: string,
  ownerId: string,
