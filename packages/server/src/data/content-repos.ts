...
export interface ContentRepoLike {
  ...
  /**
   * `id` is optional and defaults to a fresh one. A caller passes it when it
   * needs to reason about the row BEFORE it exists — the currency route mints
   * the id first so the exchange-anchor check has a "me" to test the chain
   * against.
   */
  create(ctx: WorldContext, input: Record<string, unknown>, id?: string): Promise<{ id: string }>
  update(
    ctx: WorldContext,
    id: string,
