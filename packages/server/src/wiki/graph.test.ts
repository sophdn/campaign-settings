...
describe('graph edges — a link inside a hidden passage is not an edge', () => {
  ...
  it('makes a TYPED relationship an edge, which it never was before chain 470', async () => {
    await withTestDatabase(async (pool) => {
      const db = createDb(pool)
      await migrateToLatest(db)
      const w = await twoPublicEntities(db, 'typed1')

      // Neither prose mentions the other, so a `description` edge cannot
      // account for this. The fifteen-type vocabulary a DM records used to be
      // invisible here — the graph and the entity pages disagreed about how
      // connected the world was.
      await createRelationship(w.owner, {
        fromId: w.crow.id,
        toId: w.hollow.id,
        type: 'ally_of',
      })

      const g = await buildEntityGraph(w.owner)
      expect(hasEdge(g.edges, w.crow.id, w.hollow.id)).toBe(true)
      expect(g.edges.find((e) => e.from.id === w.crow.id && e.to.id === w.hollow.id)?.type).toBe(
        'relationship',
      )
      // Both endpoints are public, so a player gets it too.
      expect(hasEdge((await buildEntityGraph(w.player)).edges, w.crow.id, w.hollow.id)).toBe(true)
    })
  })

  it('keeps a reveal-sourced relationship off the graph until the reveal is seen', async () => {
    await withTestDatabase(async (pool) => {
      const db = createDb(pool)
      await migrateToLatest(db)
      const w = await twoPublicEntities(db, 'typed2')

      const reveal = await createPassage(
        w.owner,
        {
          entityId: w.crow.id,
          body: 'In truth he answers to [[The Hollow Man]].',
          visibility: 'restricted',
        },
        w.ownerId,
      )
      await reconcileBrackets(w.owner, w.crow.id)

      // The relationship reaches the same outcome the bracket edge already did,
      // by the new route: `listRelationshipsForEntity` applies BOTH the
      // both-endpoints rule and the source-passage rule, so the graph inherits
      // them rather than carrying a second copy.
      expect(hasEdge((await buildEntityGraph(w.owner)).edges, w.crow.id, w.hollow.id)).toBe(true)
      expect(hasEdge((await buildEntityGraph(w.player)).edges, w.crow.id, w.hollow.id)).toBe(false)

      await grantPassageVisibility(w.owner, reveal.id, w.playerId)
      expect(hasEdge((await buildEntityGraph(w.player)).edges, w.crow.id, w.hollow.id)).toBe(true)
    })
  })
