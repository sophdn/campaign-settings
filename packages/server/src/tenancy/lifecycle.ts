 ...
 *
 * RECORDED DECISION — player-owned notes for the world are DELETED, not
 * retained. The DM can read player data for their world, so retaining it would
 * leave a departed player's private notes readable by the people they walked
 * away from. The UI offers a download before the leave button does anything, so
 * the choice is "take it with you", not "lose it silently".
 *
 * Their PC PAGE is a different thing and stays. Since migration 0018 a
 * character is a DM-authored content entity that merely POINTS at the account
 * playing it, so the world's write-up of who fought at the bridge is the DM's
 * and survives the player leaving. Only the pointer is cleared — and there is
 * deliberately no statement here that clears it. `pc_details.account_id` is
 * half of a foreign key into `world_members`, declared ON DELETE SET NULL
 * (account_id), so deleting the membership row below IS what releases the
 * character. A clause here would be a second implementation of a rule the
 * database already keeps, and the kind that rots when someone adds a third way
 * to lose a membership.
 *
